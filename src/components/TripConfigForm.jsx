...
export default function TripConfigForm({ onSubmit, initialValues }) {
  ...
  return (
    <div className="glass-card config-form">
      <h2 className="form-title">开始规划你的定制旅程</h2>
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <div style={{ marginTop: '0.6rem', fontSize: '0.85rem', display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ color: '#9ca3af' }}>日均预算: <strong style={{ color: '#fff' }}>¥{Math.round(budget / days)}</strong> /天</span>
            <span>预算等级: <strong style={{ color: tier.color }}>{tier.label}</strong></span>
          </div>
        </div>
